...
Template.cards.any_list_selected = function () {
  return !Session.equals('list_id', null);
};

Template.cards.events = {};

Template.cards.events[ okcancel_events('#new-card') ] =
  make_okcancel_handler({
    ok: function (text, evt) {
      Cards.insert({
        text: text,
...
Template.cards.cards = function () {
  ...
  return Cards.find(sel, {sort: {timestamp: 1}});
};

//sdfsdf

Template.card_item.editing = function () {
  return Session.equals('editing_cardname', this._id);
};

Template.card_item.events = {
  'click .check': function () {
    Cards.update(this._id, {$set: {done: !this.done}});
  },
  ...
  'dblclick .display .card-text': function (evt) {
    Session.set('editing_cardname', this._id);
    Meteor.flush(); // update DOM before focus
    focus_field_by_id("card-input");
  },
  ...
  'click .remove': function (evt) {
    ...
    Meteor.setTimeout(function () {
      Cards.update(id, {$pull: {tags: tag}});
    }, 300);
  },

};

Template.card_item.events[ okcancel_events('#card-input') ] =
  make_okcancel_handler({
    ok: function (value) {
      Cards.update(this._id, {$set: {text: value}});
      Session.set('editing_cardname', null);
    },
    cancel: function () {
      Session.set('editing_cardname', null);
    }
